...
def recete_cozumle(mamul_id, miktar, stok_kontrol=True):
    ...
    Yarı mamuller otomatik olarak tespit edilir.
    ...
    def _recete_ara(urun_id, kalan_miktar, derinlik=0):
        if derinlik > 10:  # Sonsuz döngü koruması
            return
        
        recete = cursor.execute("""
            SELECT R.hammadde_id, R.miktar, S.tip, S.kod, S.ad, COALESCE(S.miktar, 0) as mevcut_stok
            FROM Receteler R
        ...
        for hammadde_id, birim_miktar, tip, kod, ad, mevcut_stok in recete:
            ...
            if tip == 'MAM':
                ...
                # Recursive olarak yarı mamulün reçetesini çöz
    ...
    with t1:
        ...
        with st.form("is_f"):
            ...
            if st.form_submit_button("Üretimi Başlat"):
                ...
                # YARI MAMUL DESTEKLİ REÇETE KONTROLÜ
